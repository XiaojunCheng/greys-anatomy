...
public abstract class ReflectAdviceListenerAdapter implements AdviceListenerAdapter {
    ...
    private final ThreadLocal<GaStack<LazyGet<?>>> infoStackRef = ThreadLocal.withInitial(() -> new ThreadUnsafeGaStack<>());
    ...
    private GaMethod toMethod(ClassLoader loader, Class<?> clazz, String methodName, String methodDesc)
        ...
        if (GaCheckUtils.isEquals(methodName, "<init>")) {
            return new GaConstructorImpl(toConstructor(clazz, argsClasses));
        } else {
            return new GaMethodImpl(toMethod(clazz, methodName, argsClasses));
        }
    }
